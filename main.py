...
def run_strategy(data: pd.DataFrame, strategy, label: str) -> dict:
    print(f"\n{'─'*55}")
    ...
    # Save chart
    chart_path = f"results/{label}_report.png"
    plot_results(df, equity_df, trades_df, strategy.name, config.TICKER, chart_path)
...
def main():
    print(f"\n{'═'*55}")
    print(f"  BACKTESTER  |  {config.TICKER}  |  {config.START_DATE} → {config.END_DATE}")
    print(f"  Capital: ${config.INITIAL_CASH:,}  |  Commission: {config.COMMISSION*100}%  |  Slippage: {config.SLIPPAGE*100}%")
    print(f"{'═'*55}")

    raw_data = fetch_data(config.TICKER, config.START_DATE, config.END_DATE, config.INTERVAL)
    ...
    for label, strategy in strategies.items():
        results[label] = run_strategy(raw_data, strategy, label)
